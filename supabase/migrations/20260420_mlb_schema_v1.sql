...
begin
  if not exists (
    ...
    where t.typname = 'analysis_confidence' and n.nspname = 'public'
  ) then
...
begin
  if not exists (
    ...
    from pg_type t
    join pg_namespace n on n.oid = t.typnamespace
    where t.typname = 'pick_recommendation' and n.nspname = 'public'
  ) then
...
begin
  if not exists (
    ...
    from pg_type t
    join pg_namespace n on n.oid = t.typnamespace
    where t.typname = 'pick_result_status' and n.nspname = 'public'
  ) then
...
drop trigger if exists trg_game_inputs_set_updated_at on public.game_inputs;
create trigger trg_game_inputs_set_updated_at
before update on public.game_inputs
for each row
execute function public.set_updated_at();

drop trigger if exists trg_analyses_set_updated_at on public.analyses;
create trigger trg_analyses_set_updated_at
before update on public.analyses
for each row
execute function public.set_updated_at();

drop trigger if exists trg_picks_set_updated_at on public.picks;
create trigger trg_picks_set_updated_at
